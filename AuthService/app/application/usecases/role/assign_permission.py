...
from domain.exceptions.role import RoleAlreadyHasPermissionException, RoleNotFoundException
from domain.exceptions.identifier import InvalidIdentifierException
from domain.repositories.uow import BaseUnitOfWork
from application.dto.role import RoleAssignPermissionRequestDTO
from application.usecases.base import BaseUseCase


class BaseRoleAssignPermissionUseCase(BaseUseCase[RoleAssignPermissionRequestDTO, None]):
    ...
    
class RoleAssignPermissionUseCase(BaseRoleAssignPermissionUseCase):
    def __init__(
        self,
        uow: BaseUnitOfWork
    ):
    ...
    async def execute(self, request: RoleAssignPermissionRequestDTO) -> None:
        async with self.uow as uow:
            ...
            if permission in role.permissions:
                raise RoleAlreadyHasPermissionException(
                    role_name=role.name,
